...
import com.darekbx.dashboard.viewmodel.CurrencyViewModel
...
fun CurrencyWidget(
    ...
    waterLevelViewModel: CurrencyViewModel = hiltViewModel(),
    widget: Currency
) {
    val currencyData by waterLevelViewModel.fetchCurrencyData(widget).observeAsState()
    CurrencyWidgetCard(modifier, currencyData, widget)
}
            ...
            currencyData.forEachIndexed { index, value ->
                ...
                drawIntoCanvas {
                    ...
                    it.nativeCanvas.drawText(
                        "%.2f".format(minValue),
                        0F,
                        (maxValue - minValue) * heightRatio + 8F,
                        paint
                    )
                    if (showLatest) {
                        it.nativeCanvas.drawText(
                            "%.2f".format(latest),
                            0F,
                            (maxValue - latest) * heightRatio + 8F,
                            paint
                        )
                    }
                }
